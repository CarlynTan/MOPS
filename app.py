...
def load_prices():
    engine = get_engine()
    syms_tw  = "','".join([f"{k}.TW"  for k in WATCHLIST])
    syms_two = "','".join([f"{k}.TWO" for k in WATCHLIST])
    df = pd.read_sql(f"SELECT date, symbol, open, close FROM stock_prices WHERE symbol IN ('{syms_tw}','{syms_two}') ORDER BY symbol, date", engine)
...
def render_rev_table(base_df, rev_col, unit_label, s3a, s6a, s3y, s6y, fmt="0f"):
    tbl = base_df.sort_values(["company","date"])
    ...
    num_fmt = lambda x: (f"{x:,.2f}" if fmt=="2f" else f"{x:,.0f}") if pd.notna(x) else ""
    pct_fmt = lambda x: f"{x:.1f}%" if pd.notna(x) else ""
    tbl[rev_col]       = tbl[rev_col].apply(num_fmt)
    tbl["3M Avg Rev"]  = tbl["3M Avg Rev"].apply(num_fmt)
    tbl["6M Avg Rev"]  = tbl["6M Avg Rev"].apply(num_fmt)
    for col in ["yoy_pct","mom_pct","3M Avg YoY%","6M Avg YoY%"]:
        if col in tbl.columns:
            tbl[col] = tbl[col].apply(pct_fmt)
    show = ["company_full","date",rev_col]
    ...
    out = tbl[show].rename(columns={"company_full":"Company","date":"Sort Date",rev_col:f"Revenue ({unit_label})","yoy_pct":"YoY %","mom_pct":"MoM %"})
...
with st.sidebar:
    ...
    selected_years  = st.multiselect("Filter by year",  options=available_years,  default=available_years,  format_func=str)
    selected_months = st.multiselect("Filter by month", options=available_months, default=available_months, format_func=lambda x: MONTH_NAMES[x])
    ...
    default_options = available
    selected = st.multiselect("Select stocks", options=available, default=default_options, format_func=lambda x: WATCHLIST_DISPLAY[x])
    if not selected:
        st.warning("Please select at least one stock.")
        st.stop()
    st.divider()
    group_sum = st.toggle("📊 Show sector total (sum selected)", value=False,
                          help="Aggregate revenue across all selected companies. Useful for sub-sector analysis.")
    st.divider()
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
    ...
    "📈 Growth Momentum", "📉 3M Avg YoY", "📉 6M Avg YoY", "🔗 Price vs Fundamentals",
])
...
with tab3:
    ...
    if fx_df.empty:
        ...
        b3_twd = aggregate_if_needed(base3.copy(), "rev_current", group_sum)
        b3_usd = aggregate_if_needed(base3.copy(), "rev_usd", group_sum)
        f3_twd = apply_date_filter(b3_twd)
        f3_usd = apply_date_filter(b3_usd)
        if f3_twd.empty:
            st.warning("No data.")
        else:
            cl, cr = st.columns(2)
            with cl:
                ...
                st.plotly_chart(make_chart(f3_twd,"date","rev_current","company",{"rev_current":"TWD k","date":"Month"},ct3), use_container_width=True, key="pc3l")
            with cr:
                st.markdown("**Revenue in USD (millions)**")
                st.plotly_chart(make_chart(f3_usd,"date","rev_usd","company",{"rev_usd":"USD mn","date":"Month"},ct3), use_container_width=True, key="pc3r")
...
with tab5:
    ...
    if f5.empty:
        ...
        t5 = t5.rename(columns={"company_full":"Company","date":"Sort Date","rev_current":"Revenue (TWD k)","3M Avg Rev":"3M Avg Rev (TWD k)","3M Avg YoY%":"3M Avg YoY%","mom_pct":"MoM %"})
...
with tab6:
    ...
    if f6.empty:
        ...
        t6 = t6.rename(columns={"company_full":"Company","date":"Sort Date","rev_current":"Revenue (TWD k)","6M Avg Rev":"6M Avg Rev (TWD k)","6M Avg YoY%":"6M Avg YoY%","mom_pct":"MoM %"})
...
with tab7:
    ...
    if len(selected) == 1:
        ...
        if merged.empty:
            ...
            price_view = st.radio("Price return", ["MoM Change","3M Change","Both"], horizontal=True, key="pv")
            max_abs = max(
            ...
            fig7.update_layout(
                ...
                yaxis2=dict(title="Price Return %", overlaying="y", side="right", gridcolor="#eeeeee", range=price_range, zeroline=True, zerolinecolor="#cccccc"),
